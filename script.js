...
function formInputsAreValid() {
    ...
    const movieErrors = movieTitle.length >= 2 ? console.log("true") : alert('Movie titles must be at least 2 characters.');
    ...
    if (validValues) {
        ...
        const spinalTap = movieRating >= 11 ? alert("Only Spinal Tap can be 11") : console.log("bad");
    ...
    return ("Movie Title " + validValues);

}


function ratingInput(inRating) {


    let outRating = (inRating);

    return ("Sky's Movie Rating: " + outRating);
};
...
$("#button-add").on("click", function (event) {
    ...
    if (formInputsAreValid()) {
        ...
        //  </div>
        let movieDetails = (`Move Title: `) + `"${$("#movie-title").val()}"&nbsp;&nbsp;`
        movieDetails = movieDetails + `  ${ratingInput($("#movie-rating").val())}`

        let $movieDiv = $("<div>").html(movieDetails).addClass("movie-details");
        $("<button>").text("REMOVE").addClass("rmvbutton").appendTo($movieDiv);
...
$(".movies").on("click", ".rmvbutton", function () {
    ...
    //  no event.target root is needed.
